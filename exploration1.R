...
# MA <- read.csv("C:/Users/eliecer.diaz/Documents/Recruitment/FinalmenteData.csv", 
#                    header = TRUE,
#                    sep = ",")
...
MA <- read.csv("/home/elvi/Documents/MegaSync/Recruitment/FinalmenteData.csv", 
                   ...
                   sep = ",")
View(MA)
MA$Biomass <- as.numeric(MA$Biomass)
##############
#Houskeeping
##############
#View(MA)
...
#########################
library(stringr)
MA1$Lat <- str_sub(MA1$Lat, start=1, end=-6)
MA1$Lat <- gsub(",", ".",MA1$Lat)
MA1$Lat <- as.numeric(MA1$Lat)

MA1$Lon <- str_sub(MA1$Lon, start=1, end=-6)
...
# we need to do conditional boxplots
# Boxplots next to each other implies no collinearity.
# If the boxplots differ per category then there may be collinearity.

# par(mfrow = c(1, 3), cex.lab = 1.5, mar = c(6,5,2,2))

# # YEAR
 boxplot(zerofive ~ year , 
         xlab = "year",
         ylab = "recruits",
         data = MA1)
# # yes

boxplot(zerofive ~ chlapr, 
         xlab = "Chl_april",
         ylab = "recruits",
         data = MA1)
# #yes

 boxplot(zerofive ~ chlsumer , 
         xlab = "Chl_summer",
         ylab = "recruits",
         data = MA1)
# #yes
# #########################################################
# boxplot(zerofive ~ site , 
#         xlab = "year",
#         ylab = "recruits",
#         data = MA1)
# # yes

# boxplot(recr_aut ~ chlapr, 
#         xlab = "Chl_april",
#         ylab = "recruits",
#         data = MA1)
# #yes
# dev.new()
# boxplot(recr_aut ~ chlsumer , 
#         xlab = "Chl_summer",
#         ylab = "recruits",
#         data = MA1)
# #yes

 ggplot(data = MA1, mapping = aes(x = year, y = zerofive)) +
...
# ###################################################3#############
# ## YEAR 2006 chlorophyl a messed up the prediction
# #Temperature
 plot(MA1$temp10,MA1$sal10, 
         xlab = "Temp_0",
         ylab = "Salinity 0",
         col = MA1$year,
         data = MA1)
# plot(MA1$temp0,MA1$zerofive, 
#         col = 1,
#         xlab = "Temp_0",
#         ylab = "recruits",
#         data = MA1)
...
# #yes
# ggplot(data = MA1, aes(x = temp10, y = zerofive)) +
#     geom_point() + facet_wrap(.~ year) + geom_smooth(method = "loess")

# ggplot(data = MA1, aes(x = sal0, y = zerofive)) +
#     geom_point() + facet_wrap(.~ year) + geom_smooth(method = "loess")
...
MA1 <- MA2
#######################################
# ggplot(data = MA1, mapping = aes(x = mean, y = zerofive)) +
#     geom_boxplot(alpha = 0) + facet_wrap(.~ year) +
#     geom_jitter(alpha = 0.3, color = "tomato")



# #Salinity
# plot(MA1$year ,MA1$zerofive,
#         xlab = "Salinity",
#         ylab = "recruits",
#         type = "h",
#         col = MA1$year,
...
# plot(MA1$secchi, MA1$zerofive,
#         xlab = "Secchi",
#         ylab = "recruits",
#         col = MA1$year,
#         data = MA1)

# ggplot(data = MA1, aes(x = secchi, y = zerofive)) +
#     geom_point() + facet_wrap(.~ year) + geom_smooth(method = "lm")

# dev.new()
# ggplot(data = MA1, mapping = aes(x = secchi, y = zerofive)) +
#      facet_wrap(.~ year) +
#     geom_jitter(alpha = 0.3, color = "tomato")        
# # yes
# MA1$secchi
...
# ggplot(data = MA1, aes(x = mean_secchi, y = zerofive)) +
#     geom_point() + facet_wrap(.~ year) + geom_smooth(method = "lm")


# #Biomass
# plot(MA1$Biomass, MA1$zerofive, 
#         xlab = "Biomass",
#         ylab = "recruits",
#         col = MA1$year,

#         data = MA1)
# p2 <- ggplot(MA1, aes(x =Biomass, y = zerofive))
# p2 <- p2 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p2 <- p2 + geom_smooth(method = "loess") + facet_wrap( ~ year, nrow = 1)
# p2
# names(MA1)
# p2 <- ggplot(MA1, aes(x = Isaeus, y = zerofive))
# p2 <- p2 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p2 <- p2 + geom_smooth(method = "lm") + facet_wrap( ~ year, nrow = 1)
# p2
# plot(MA1[MA1$year == "2004",]$Isaeus)
# points(MA1[MA1$year == "2007",]$Isaeus, col = 3)
# #########################3 Smallers
# library(gridExtra)
# p1 <- ggplot(MA1, aes(x = year, y = recr_aut))
# p1 <- p1 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p1 <- p1 + geom_smooth(method = "lm") + facet_wrap( ~ year, nrow = 1)
# p1
# grid.arrange(p1, p3, nrow = 2)

# p3 <- ggplot(MA1, aes(x = year, y = zerofive))
# p3 <- p3 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p3 <- p3 + geom_smooth(method = "lm") + facet_wrap( ~ year, nrow = 1)
# p3


# p4 <- ggplot(MA1, aes(x = two, y = zerofive))
# p4 <- p4 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p4 <- p4 + geom_smooth(method = "lm") + facet_wrap( ~ year, nrow = 1)

# p5 <- ggplot(MA1, aes(x = one, y = zerofive))
# p5 <- p5 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p5 <- p5 + geom_smooth(method = "lm") + facet_wrap( ~ year, nrow = 1)

# grid.arrange(p1, p3, p4, p5, nrow = 4)
# ########## Temprature
# p4 <- ggplot(MA1, aes(x = temp10, y = zerofive))
# p4 <- p4 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p4 <- p4 + geom_smooth(method = "loess") + facet_wrap( ~ year, nrow = 1)


# p5 <- ggplot(MA1, aes(x = temp10, y = zerofive))
# p5 <- p5 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p5 <- p5 + geom_smooth(method = "loess") + facet_wrap( ~ year, nrow = 1)


# grid.arrange(p4, p5, nrow = 2)
# ################# SALINTIY
# p6 <- ggplot(MA1, aes(x = sal0, y = zf_log))
# p6 <- p6 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p6 <- p6 + geom_smooth(method = "lm") + facet_wrap( ~ year, nrow = 1)
# p6

# p7 <- ggplot(MA1, aes(x = sal10, y = zf_log))
# p7 <- p7 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p7 <- p7 + geom_smooth(method = "loess") + facet_wrap( ~ year, nrow = 1)
# #p7
# grid.arrange(p6, p7, nrow = 2)

# MA1$diff_sal = MA1$sal10 - MA1$sal0
# plot(diff_sal)

# p8 <- ggplot(MA1, aes(x = Lat, y = zf_log))
# p8 <- p8 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p8 <- p8 + geom_smooth(method = "loess") + facet_wrap( ~ year, nrow = 1)
# p8
# dev.new()
# p9 <- ggplot(MA1, aes(x = Biomass, y = zerofive))
# p9 <- p9 + geom_point(aes(color=factor(year))) +  scale_fill_grey()
# p9 <- p9 + geom_smooth(method = "lm") + facet_wrap( ~ year, nrow = 1)
# grid.arrange(p8, p9, nrow = 2)
# p9
# # SITE

# g1 <- ggplot(data=MA1, aes(x=site, y= zerofive, fill=site)) +
#     geom_bar(stat="identity") + facet_wrap( ~ year, nrow = 1)
# g1
# library(tidyverse)
# library(forcats)
# MA1 %>%
#     #count(Position) %>%
#     mutate(site = fct_reorder(site, zerofive,  .desc = TRUE)) %>%
#     ggplot(aes(x = site, y = zerofive)) +
#       geom_bar(stat = 'identity') +
#        facet_wrap(.~ year) +
#        theme(text =  element_text(size = 13), axis.text.x = element_text(angle=90, hjust=0))
...
# g2 <- ggplot(data=MA1, aes(x=site, y= two, fill=site)) +
#     geom_bar(stat="identity") + facet_wrap( ~ year, nrow = 1)
# g2
# g3 <- ggplot(data=MA1, aes(x=site, y= four, fill=site)) +
#     geom_bar(stat="identity") + facet_wrap( ~ year, nrow = 1)
# grid.arrange(g1, g2, g3, nrow = 3)

save(MA1, file = "C:/Users/eliecer.diaz/Documents/Recruitment/MA1.Rda")
###################################################################
...
# summary(NB)
library(mgcv)
# GAM2 <- gamm(zerofive ~  temp0.std*mean + s(Biom.std, by = year) + mean_secchi*Isa.std, # by = Region), # bs = "cr")  
#                  data = MA1,
#                 family = poisson, #1.64Gamma(link = log),
#                 correlation = corExp(form = ~ Xkmr + Ykmr, nugget = TRUE))

# GAM2 <- gamm(zerofive ~  s(temp0.std, by = mean) +
#                          s(Biom.std, by = mean) +
#                          mean*Isa.std, # by = Region), # bs = "cr")  
#                  data = MA1,
#                 family = poisson, #1.64Gamma(link = log),
#                 correlation = corExp(form = ~ Xkmr + Ykmr, nugget = TRUE))

# GAM3 <- gamm(zerofive ~  s(temp0.std, by = mean, bs = "cr") +
#                          s(Biom.std, by = mean) +
#                          mean*Isa.std, # by = Region), # bs = "cr")  
#                  data = MA1,
...
# GAM6 <- gamm(zerofive ~  s(temp0.std, by = mean, bs = "cs") +
#                          s(Biom.std),
#                          data = MA1,
#                 family = neg.bin(theta = c(1.94)), #1.64Gamma(link = log),
#                 correlation = corExp(form = ~ Xkmr + Ykmr, nugget = TRUE))

# GAM7 <- gamm(zerofive ~  s(temp0.std, by = mean, bs = "cr"),
#                          data = MA1,
#                 family = neg.bin(theta = c(2.3)), #1.64Gamma(link = log),
#                 correlation = corExp(form = ~ Xkmr + Ykmr, nugget = TRUE))

# GAM8 <- gamm(zerofive ~  s(temp0.std, by = mean, bs = "cr") + Isaf,
#                          data = MA1,
#                 family = neg.bin(theta = c(2.3)), #1.64Gamma(link = log),
#                 correlation = corExp(form = ~ Xkmr + Ykmr, nugget = TRUE))


# hist(MA1[MA1$year == 2004,]$Isa.std, 30)
# hist(MA1[MA1$year == 2005,]$Isa.std, 30)
# hist(MA1[MA1$year == 2006,]$Isa.std, 30)
# hist(MA1[MA1$year == 2007,]$Isa.std, 30)

# #Only 900 observation and 1 column--> we need a cluster very soon!
# AIC(GAM3$lme, GAM4$lme, GAM5$lme, GAM8$lme) # no need two, pick 3
...
mesh <- inla.mesh.2d(boundary = bound, 
                      max.edge = c(1000), #max triangle edge
                      cutoff   = 10) #minimum distance allowed between points
mesh$n
# 342
...
Xm <- model.matrix(~  Biom.std*mean, 
                   data = MA1)
colnames(Xm)
dim(MA1)
dim(Xm)

X <- data.frame(temp0.std = MA1$temp0.std,
                 mean = MA1$mean)
dim(X)
...
f3 <- y ~ -1 + Intercept + f(temp0.std, model = "rw1", constr = FALSE) +
                           + Biom.std + mean + Biom.std:mean + f(w, model = spde)

f3a <- y ~ -1 + Intercept + f(temp0.std, model = "rw1", constr = FALSE) +
                           + Biom.std + mean + f(w, model = spde)
f3b <- y ~ -1 + Intercept + f(temp0.std, mean, model = "rw1", constr = FALSE) +
                           + f(w, model = spde)
f3c <- y ~ -1 + Intercept + f(temp0.std, mean, model = "rw1", constr = FALSE) +
                           + f(w, model = spde)# <-----Chosen
                           
f3cS <- y ~ -1 + Intercept + f(temp0.std, mean, model = "rw2",scale.model=TRUE, constr = FALSE, hyper = hyper.prec) +
                           + f(w, model = spde)# <-----Chosen
...
dic2  <- c( I3$dic$dic, I3a$dic$dic,
            I3b$dic$dic, I3c$dic$dic, I3cS$dic$dic, I4$dic$dic)
waic2 <- c(I3$dic$dic, I3a$dic$dic,
            I3b$dic$dic, I3c$dic$dic, I3cS$dic$dic, I4$dic$dic)

Z     <- cbind(dic2, waic2)
rownames(Z) <- c( "SmootherNB GLM + SPDE",
                  "NB + SPDE")
Z # smoother NF + SPDE
...
theta = 3.7
Fit3 <- I3cS$summary.fitted.values[1:595, "mean"]
theta <- I3cS$summary.hyperpar[1, "mean"]
...
Fit1 <- I3cS$summary.fitted.values[1:595, "mean"]
MA1$Fitted1 <- Fit1
MA1$Fit1.025 <- I3cS$summary.fitted.values[1:595, "0.025quant"]
MA1$Fit1.975 <- I3cS$summary.fitted.values[1:595, "0.975quant"]
...
w.pm <- I3cS$summary.random$w$mean 
w.sd <- I3cS$summary.random$w$sd
...
f3cS <- y ~ -1 + Intercept +
           f(temp0.std, mean, model = "rw1", scale.model = TRUE, constr = FALSE) +
                           + f(w, model = spde)# <-----Chosen
I5 <- inla(f5,
           family = "nbinomial", 
...
# And compare the two models with DICs and WAICs
dic2  <- c(I3cS$dic$dic, I5$dic$dic)
waic2 <- c(I3cS$waic$waic,I5$waic$waic )
Z     <- cbind(dic2, waic2)
...
p <- p + geom_text(aes(x = Lon, y = Lat, label = site),
                  data = MA1,
                  size=2,
                  color = "black") 
p
